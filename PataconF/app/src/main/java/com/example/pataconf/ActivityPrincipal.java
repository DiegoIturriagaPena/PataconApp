...
public class ActivityPrincipal extends AppCompatActivity implements View.OnClickListener {
    ...
    private void cargarVista(DocumentSnapshot document) {
        ...
        usuario.setPerfil(document.getString("perfil"));

        this.nombreUsuario.setText(usuario.getNombre() + " " + usuario.getApellido());
        this.tipoUsuario.setText(usuario.getPerfil());
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    if (task.isSuccessful()) {
                        ...
                        if (document.exists()) {
                            ...
                            Usuario usu = new Usuario(user.getUid().toString(),nombre,apellido,correo,telefono,perfil,key);
